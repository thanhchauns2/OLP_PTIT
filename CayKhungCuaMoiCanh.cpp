...
void take_level(int node, int pre, int lvl) {
	level[node] = lvl;
	for (auto x : adj[node])
	{
		if (x == pre) continue;
		take_level(x, node, lvl + 1);
	}
}

void build_sparse(int v, int p) {
	sparse[v][0] = edges[v][p];
	for (int i = 1; i <= l; i++) {
//		if (sparse[v][i] == -1) sparse[v][i] = edges[v][p];
		sparse[v][i] = max(sparse[v][i - 1], sparse[up[v][i - 1]][i - 1]);
	}
	for (auto x : adj[v]) {
		if (x == p) continue;
		build_sparse(x, v);
...
ll max_segment(int node, int lca) {
	int dist = level[node] - level[lca];
	int ans = 0;
	if (dist == 0) return 0LL;
	FOR(i, 0, 25)
	{
		if (((1LL << i) & dist) == 0) continue;
//		cout << node << ' ' << lca << ' ' << dist << '\n';
//		cout << sparse[node][i] << ' ' << i << '\n';
		ans = max(ans, sparse[node][i]);
		node = up[node][i];
	}
	return ans;
}
...
void Process() 
{
	...
	while (!PQ.empty())
	{
		...
		if (k1 != q1)
		{
			edges[PQ.top()[1]][PQ.top()[2]] = PQ.top()[0];
			edges[PQ.top()[2]][PQ.top()[1]] = PQ.top()[0];
			adj[PQ.top()[1]].pb(PQ.top()[2]);
			adj[PQ.top()[2]].pb(PQ.top()[1]);
			total += PQ.top()[0];
			ll k = dsu[PQ.top()[1]];
			ll q = dsu[PQ.top()[2]];
			if (k > q) swap(k, q);
...
//			cout << PQ.top()[0] << ' ' << PQ.top()[1] << ' ' << PQ.top()[2] << '\n';
	...
	cout << '\n';
	preprocess(0);
	sparse.resize(n + 5, vec<int> (l + 5, -1));
	take_level(0, 0, 0);
	build_sparse(0, 0);
//	FOR(i, 0, n - 1)
//	{
//		FOR(j, 0, 5) cout << sparse[i][j] << ' ';
//		cout << '\n';
//	}
//	cout << lca(1, 3) << '\n';
	for (auto seg : store)
	{
		if (edges[seg[1]][seg[2]] != 0)
		{
			cout << total << '\n';
			continue;
		}
		ll L = lca(seg[2], seg[1]);
		ll mx1 = max_segment(seg[2], L);
		ll mx2 = max_segment(seg[1], L);
//		cout << seg << ' ' << max(mx1, mx2) << '\n';
		cout << total - max(mx1, mx2) + seg[0] << '\n';
	}
}
