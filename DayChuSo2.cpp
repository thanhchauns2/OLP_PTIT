...
void Process()
{
	cin >> s;
	len = strlen(s);
	FORD(i, len, 1) s[i] = s[i - 1];
	FOR(j, 0, 9) NEXT[len + 1][j] = len + 1;
	FORD(i, len, 1)
	{
		FOR(j, 0, 9) NEXT[i][j] = NEXT[i + 1][j];
		NEXT[i][s[i] - '0'] = i;
	}
	...
	FORD(i, len, 0)
	{
		...
		FOR(j, 0, 9)
		{
			...
			if(id <= len)
			{
				if(dp[id] < dp[i]) dp[i] = dp[id], re[i] = j;
			}
			else 
			{
				dp[i] = -1;
