...
bool isPrime(ull a, int iter = 5) 
{
	...
	{
        ull a = 2 + rand() % (a - 3);
        if (powermod(a, a - 1, a) != 1) return false;
    }
    return true;
}
...
void Process()
{
	...
	vec<set<p<ll, ll> > > label(a + 5);
	map<int, map<int, int> > dist;
	function<ll(ll, ll)> dfs = [&](ll node, ll pre)
	{
		if (C[node].size() == 1) return 0LL;
		ll ans = 0;
		for (auto x : C[node])
		{
			if (x == pre) continue;
			ll k = dfs(x, node) + 1;
			ans = max(ans, k);
			label[node].insert(make_pair(k, x));
			dist[node][x] = k;
			dist[x][node] = k;
		}
		return ans;
	...
	{
		ll k = dfs(x, 1) + 1;
		label[1].insert(make_pair(k, x));
		dist[1][x] = k;
		dist[x][1] = k;
	}
	function<void(ll, ll)> dfs2 = [&](ll node, ll pre)
	{
		for (auto x : C[node])
		{
			if (x == pre) continue;
//			ll k = dist[node][x];
//			label[x].insert(make_pair(k + 1, node));
			auto k = label[node].rbegin();
//			cout << node << ' ' << x << ' ' << pre << ' ' << k->s << '\n';
			if (k->s == x)
			{
				if (label[node].size() > 1) ++k;
				else
				{
					label[x].insert(make_pair(1, node));
					dfs2(x, node);
					continue;
				}
			}
			ll k1 = k->f;
			label[x].insert(make_pair(k1 + 1, node));
			dfs2(x, node);
		}
	};
	dfs2(1, 1);
	...
	FOR(i, 1, a)
	{
		ans = max(ans, label[i].rbegin()->f);
	}
	cout << ans << '\n';
//	FOR(i, 1, a)
//	{
//		for (auto x : label[i]) cout << x << ' ';
//		cout << '\n';
//	}
}
