...
Matrix(ll) add(Matrix(ll) A, Matrix(ll) B)
{
	ll a = A.size();
	FOR(i, 0, a - 1)
	{
...
Matrix(ll) mulMatrix(Matrix(ll) A, ll b)
{
	if (b == 1) return A;
	Matrix(ll) B = mulMatrix(A, b / 2);
	B = mul(B, B);
	if (b & 1) B = mul(B, A);
	return B;
}

Matrix(ll) extract(Matrix(ll) A, ll b)
{
	if (b == 1) return A;
	Matrix(ll) X = extract(A, b / 2);
	Matrix(ll) B = mulMatrix(A, b / 2);
	B = add(B, I);
	B = mul(B, X);
	if (b & 1) B = add(B, mulMatrix(A, b));
	return B;
}

void print(Matrix(ll) A)
{
	ll a = A.size();
	FOR(i, 0, a - 1)
	{
